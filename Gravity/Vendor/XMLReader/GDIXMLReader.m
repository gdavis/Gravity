...
NSString *const kGDIXMLReaderTextNodeKey = @"text";
NSString *const kGDIXMLReaderAttributePrefix = @"";

@interface GDIXMLReader () {
...
@implementation NSDictionary (GDIXMLReaderNavigation)
...
+ (NSDictionary *)dictionaryForPath:(NSString *)path error:(NSError **)errorPointer
{
    ...
    NSDictionary *rootDictionary = [GDIXMLReader dictionaryForXMLData:data error:errorPointer];
...
{
    GDIXMLReader *reader = [[GDIXMLReader alloc] initWithError:error];
...
+ (NSDictionary *)dictionaryForXMLString:(NSString *)string error:(NSError **)error
{
    ...
    return [GDIXMLReader dictionaryForXMLData:data error:error];
}
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
    ...
    for (NSString *key in attributeDict) {
        [childDict setValue:[attributeDict objectForKey:key]
                     forKey:[NSString stringWithFormat:@"%@%@", kGDIXMLReaderAttributePrefix, key]];
    }
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([textInProgress length] > 0)
    {
        ...
        {
            [dictInProgress setObject:textInProgress forKey:kGDIXMLReaderTextNodeKey];
        }
        else
        {
